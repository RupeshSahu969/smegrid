...
const ExploreServices = () => {
  ...
  useEffect(() => {
    ...
    if (data) {
      // Ensure full image URL if it's a relative path
      const imgUrl = data.img && data.img.startsWith('/')
        ? `http://localhost:5000${data.img}`
        : data.img;

      setServiceData({ ...data, img: imgUrl });
    }
  }, []);
          ...
          {serviceData.img && (
            ...
            >
              <img
                ...
                onError={(e) => {
                  console.error('Image failed to load:', serviceData.img);
